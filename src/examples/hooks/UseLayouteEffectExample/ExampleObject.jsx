  ...
  return (
    <div>
      ...
      <Paragraph>
        ...
        большим текстом. После чего выполняются хуки <code>useEffect</code> и
        <code>useLayoutEffect</code>в которых, 2 переменные заполняются
        маленьким текстом. Тоже самое происходит при клике на кнопку. Мы можем
        видеть, что левый блок, который использует useEffect мерцает. Это из-за
        того, что выводится начальное значение(большой текст), а затем в
        переменную помещается маленький текст. Второй блок выводится корректно,
        там используется <code>useLayoutEffect</code>.
        <code>useLayoutEffect</code> блокирует вывод реального дом, выполняет
        <code>setState</code> и после обновления корректного текста отображает в
        реальном DOM.
      </Paragraph>
      {show && (
        <div>
